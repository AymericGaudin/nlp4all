...
def make_pandas_df(cat_list):
        ...
        tweet_text = list(tweet.words for tweet in tweets)
        full_text = list(tweet.full_text for tweet in tweets)
        tweet_text = [' '.join(map(str, tweet_text[i]))for i in range(len(tweet_text))] # join words back to text
